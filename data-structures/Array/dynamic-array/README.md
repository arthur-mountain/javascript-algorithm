...
| ------------------- | ------------------------- | --------------------- | ------------ | ------------------------------------- |
| **隨機存取**        |                           |                       |              |                                       |
| get(i) / set(i)     | O(1)                      | O(1)                  | 0            | 繼承自 Static Array，無差異           |
| **搜尋**            |                           |                       |              |                                       |
| findIndex(val)      | O(n)                      | O(1)                  | 0            | 線性掃描，繼承自 Static Array         |
| **插入**            |                           |                       |              |                                       |
| insert(0, val)      | O(n)                      | O(1) 攤銷 / O(n) 最壞 | n            | 移動 + 可能擴容                       |
| insert(mid, val)    | O(n)                      | O(1) 攤銷 / O(n) 最壞 | n/2 (avg)    | 移動 + 可能擴容                       |
| push(val)           | **O(1) 攤銷** / O(n) 最壞 | O(1) 攤銷 / O(n) 最壞 | 0            | 滿時擴容為 capacity × growthFactor    |
| **刪除**            |                           |                       |              |                                       |
| delete(0)           | O(n)                      | O(1) 攤銷 / O(n) 最壞 | n-1          | 移動 + 可能縮容                       |
| delete(mid)         | O(n)                      | O(1) 攤銷 / O(n) 最壞 | n/2 (avg)    | 移動 + 可能縮容                       |
| pop()               | **O(1) 攤銷** / O(n) 最壞 | O(1) 攤銷 / O(n) 最壞 | 0            | length ≤ capacity/4 時縮為 capacity/2 |
| **內部操作**        |                           |                       |              |                                       |
| resize(newCapacity) | O(n)                      | O(n)                  | n            | 分配新陣列並複製所有有效元素          |
...
- ✅ **vs Static Array**：push/pop 不會因為「滿了」而失敗，會自動擴容
- ⚠️ **代價**：偶爾會有 O(n) 的 resize 成本，但透過倍增策略攤銷為 O(1)
- 📦 **空間浪費**：capacity 可能是 length 的 2-4 倍（trade-off）
