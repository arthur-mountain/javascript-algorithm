...
- 複習1：2026-07-19 | 耗時：5分鐘 | 獨立完成：是 | 順暢度：流暢

  - 再想到找最大公因數時自動想到輾轉相除法，在寫迴圈版的輾轉相除法時有停下來思考一下（不是硬背），

    有想到要把區間持續縮小且公因數也要能整除餘數(b = a % b)，把 b 換成餘數。

    即，因為任何能同時整除 a、b 的數，也一定能整除其餘數(a % b)，所以問題可以換成更小的一組數，答案不變。
