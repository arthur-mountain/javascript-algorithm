...
- 複習5(DFS)：2026-03-28 | 耗時：5分鐘 | 獨立完成：是 | 順暢度：流暢

  - 同時理解到我寫的方式大都是 preorder。 因為我目前的思考邏輯在看到題目當下，沒特別思考到前、中、後序，而是知道二元樹可以用 DFS/BFS，再下一步如果用 DFS 會為想需要哪些狀態和參數，遞和歸分別處理什麼，歸需要回傳什麼還是用其他變數累計，就不用回傳答案等等。但其實這題也可以用 inorder or postorder
