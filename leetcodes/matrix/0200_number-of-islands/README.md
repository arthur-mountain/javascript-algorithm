  ...
  - 若在不同集合：需要合併
    ...
    - 高度更新規則：
      ...
      - 不同高度合併：`rank[較高] 不變`（因為最長路徑沒有變長）
        ...
        - 舉例：原本一顆高度為1、一顆高度為2的樹，矮樹接高樹，其高度仍是2，所以才高度不變
      ...
      - 相同高度合併：`rank[新根] = 原高度 + 1`（因為最長路徑增加 1）
        ...
        - 舉例：原本左邊高度皆為一的樹接到另一棵樹時，其高度變2，所以才高度相同時要加一
...
- 複習3：2025-11-16 | 耗時：30分鐘(邊講後實作UnionFind) | 獨立完成：否 | 順暢度：卡頓。

  - 刻意練習 UnionFind，70%左右流暢，概念是理解得，像是 UnionFind constructor、findRoot 和 union 等實作大部分都有寫出來但實作過程有點像硬背，待複習

- 複習4：2025-11-19 | 耗時：11分鐘(BFS) | 獨立完成：是 | 順暢度：流暢。

  - 整體流程，中間在 mark land to water 的地方寫錯有卡住一下，最後有自己找出問題

- 複習5：2025-11-26 | 耗時：27分鐘(UnionFind) | 獨立完成：否 | 順暢度：流暢。

  - 跟「複習3」比較，比較沒有硬背，過程中整個大方向得思考邏輯是有先出來的然後才實作，有些小地方判斷沒想到或注意到甚至還有 typo 導致卡很久。非致命性不理解但不熟，待複習

- 複習6(BFS)：2025-12-26 | 耗時：7分鐘 | 獨立完成：是 | 順暢度：流暢。

  - 有把問題情境進行分析並抽象映射出來後才實作，沒有刻意回想或背答案

- 複習7(BFS)：2026-03-29 | 耗時：N/A(實作細節是從其他 island 題目複製過來的，因為核心概念有釐清出來，所以就沒在重新撰寫) | 獨立完成：是 | 順暢度：流暢。
